...
export class HomeComponent implements OnInit {
  ...
  constructor(
    private POKEMONserice: PokemonsService,
    private POKEMONZINHOserice: PokemonService
  ) {}

  ngOnInit(): void {
    this.POKEMONserice.pokemons('pokemon').subscribe((res: any): any => {
  ...
  mostraFire() {
    ...
    this.ativo3 = false;
    this.ativo4 = false;
    this.POKEFILTRADO = this.POKEMONS.filter(
      (t: any) =>
        t.types[0].type.name === 'fire' || t.types[1]?.type.name === 'fire'
    );
  ...
  mostraEletric() {
    ...
    this.POKEFILTRADO = this.POKEMONS.filter(
      (t: any) =>
        t.types[0].type.name === 'eletric' ||
        t.types[1]?.type.name === 'eletric'
    );
  }
  ...
  mostraWater() {
    ...
    this.POKEFILTRADO = this.POKEMONS.filter(
      (t: any) =>
        t.types[0].type.name === 'water' || t.types[1]?.type.name === 'water'
    );
  }
}
